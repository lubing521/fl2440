...
OS_STK Task_Win_Stk[TaskStkLeath];
...
void TaskStart (void *pdata)
{
  ...
  // OSTaskCreate(Task_LCD, 0, &Task_LCD_Stk[TaskStkLeath - 1], NormalTaskPrio+7);
    OSTaskCreate(Task_Win, 0, &Task_Win_Stk[TaskStkLeath - 1], NormalTaskPrio+9);
...
static void cbForewin(WM_MESSAGE *pMsg)
{	
	switch (pMsg->MsgId)
	{
		case WM_PAINT:
...
void Task_Win(void *pdata) 
{
  ...
  while(1){
  	win=WM_CreateWindow(10,10,100,100,WM_CF_SHOW,cbForewin,0);
