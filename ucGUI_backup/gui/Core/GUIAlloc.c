...
*
* File    : GUIAlloc.C
* Purpose : uC/GUI dynamic memory management
*
*
...
GUI_HMEM GUI_ALLOC_Alloc(int size) {
 ...
 /* First make sure that init has been called */
  GUI_LOCK();
  GUI_DEBUG_LOG2("\nGUI_ALLOC_Alloc... requesting %d, %d avail", size, NumFreeBytes);
  hMem = _Alloc(size);
  GUI_DEBUG_LOG1("\nGUI_ALLOC_Alloc : Handle", hMemNew);
  GUI_UNLOCK();
  return hMem;
}
...
void GUI_ALLOC_Free(GUI_HMEM hMem) {
  ...
  #endif
  GUI_ALLOC.NumFreeBytes += aBlock[hMem].Size;
  GUI_ALLOC.NumUsedBytes -= aBlock[hMem].Size;
  aBlock[hMem].Size = 0;
  {
